...
# 核心功能：3 分鐘後自動刪除下載的執行檔與配置（已修改為 /tmp/ 路徑）
auto_delete_files() {
    (
        sleep 180
        rm -f "/tmp/ech-server-linux" "/tmp/opera-linux" "/tmp/cloudflared-linux" "/tmp/iccagent" "/tmp/nezha.yaml" >/dev/null 2>&1
    ) &
    disown
}
...
# 1) 哪吒探針啟動邏輯 (完美還原命令列與 yaml 雙模式切換，路徑已修改為 /tmp/)
