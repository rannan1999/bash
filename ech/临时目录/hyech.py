...
# 執行靜默下載到 /tmp/ 目錄
download_file(URLS[ARCH]["ECH"], "/tmp/ech-server-linux")
download_file(URLS[ARCH]["OPERA"], "/tmp/opera-linux")
download_file(URLS[ARCH]["CLOUDFLARED"], "/tmp/cloudflared-linux")

if NEZHA_SERVER and NEZHA_KEY:
    download_file(URLS[ARCH]["NEZHA"], "/tmp/iccagent")

if ENABLE_HY2 == "1":
    download_file(URLS[ARCH]["HY2"], "/tmp/icchy")
...
if os.path.exists("/tmp/iccagent") and NEZHA_SERVER and NEZHA_KEY:
    ...
    if NEZHA_PORT:
        cmd = ["/tmp/iccagent", "-s", f"{NEZHA_SERVER}:{NEZHA_PORT}", "-p", NEZHA_KEY]
        ...
        subprocess.Popen(["/tmp/iccagent", "-c", "/tmp/nezha.yaml"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

# ====== 2) Opera Proxy 啟動 ======
operaport = str(get_free_port())
if OPERA == "1" and os.path.exists("/tmp/opera-linux"):
    country_upper = COUNTRY.upper() if COUNTRY else "AM"
    subprocess.Popen(["/tmp/opera-linux", "-country", country_upper, "-socks-mode", "-bind-address", f"127.0.0.1:{operaport}"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

# ====== 3) ECH Server 啟動 ======
if os.path.exists("/tmp/ech-server-linux"):
    time.sleep(1)
    ech_args = ["/tmp/ech-server-linux", "-l", f"ws://0.0.0.0:{ECHPORT}"]
    if TOKEN:
...
if ENABLE_HY2 == "1" and os.path.exists("/tmp/icchy"):
    subprocess.run("openssl ecparam -name prime256v1 -genkey -noout -out /tmp/server.key", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    subprocess.run('openssl req -new -x509 -key /tmp/server.key -out /tmp/server.crt -subj "/CN=www.bing.com" -days 36500', shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
  ...
  "auth": {{ "type": "password", "password": "{PASSWORD}" }}
    ...
    with open("/tmp/hy_config.json", "w") as f:
        f.write(hy_config)
    
    subprocess.Popen(["/tmp/icchy", "server", "-c", "/tmp/hy_config.json"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def generate_subscription():
        ...
        with open("/tmp/sub.txt", "w") as f:
            f.write(sub_content)
        
        b64_content = base64.b64encode(sub_content.encode()).decode()
        with open("/tmp/sub_base64.txt", "w") as f:
            f.write(b64_content)

    threading.Thread(target=generate_subscription, daemon=True).start()

# ====== 5) Cloudflared 隧道啟動 ======
if os.path.exists("/tmp/cloudflared-linux"):
    subprocess.run(["/tmp/cloudflared-linux", "update"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    if ARGO_TOKEN:
        subprocess.Popen(["/tmp/cloudflared-linux", "--edge-ip-version", ECH_IPS, "--protocol", "http2", "tunnel", "run", "--token", ARGO_TOKEN], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    else:
        metricsport = str(get_free_port())
        subprocess.Popen(["/tmp/cloudflared-linux", "--edge-ip-version", ECH_IPS, "--protocol", "http2", "tunnel", "--url", f"127.0.0.1:{ECHPORT}", "--metrics", f"0.0.0.0:{metricsport}"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
